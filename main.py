...
class KatanaHandler:
    ...
    def cycle_effect(self, key: str) -> None:
        ...
        if PRINT_DEBUG: print(f"[DEBUG] MIDI: Cycled {key} to {SETTINGS[key]['vals'][self.current_vals[key]]}")
        self.on_change()
    ...
    def toggle_effect(self, key: str) -> None:
        ...
        if PRINT_DEBUG: print(f"[DEBUG] MIDI: Toggled {key} {'ON' if self.active_states[key] else 'OFF'}")
        self.on_change()
...
def get_full_state_dict() -> Dict[str, Any]:
    ...
    return {
        ...
        "presets": PRESETS,
        "settings_meta": {k: ["OFF"] + SETTINGS[k]["vals"] if "sw_addr" in SETTINGS[k] else SETTINGS[k]["vals"] for k in SETTINGS}
    }
...
def update_preset_content(preset_name: str, settings: Dict[str, str]) -> None:
    ...
    if preset_name in PRESETS:
        ...
        save_config()
        if PRINT_DEBUG: print(f"[DEBUG] OP: Updated Content of Preset '{preset_name}'")
        update_ui_all()
